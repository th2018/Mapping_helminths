...
###INDEX OF PARASITE WITH COUNTRY NAMES IN THE SHP (n=4510)
```{r}
#country.dbf
...
###INDEX OF PARASITE WITH location NAMES not IN THE SHP (n=9077)
...
#index the locations which can be extracted from geocode
```{r}
...
#index the locations which can't be extracted from geocode
```{r}
d<-which(b%in%c==F)
dd<-b[d]
h_c_z[dd,]$cate<-"dd"
...
#index e: index c overlap with country shp
```{r}
dim(h_c_z[c,])
